...
if "%VisualStudioVersion%" == "" (
  for /f "delims=" %%i in ('msbuild -nologo %Root%build.proj /t:GetVsInstallRoot "/p:target=GetVsInstallRoot" /v:minimal') do set "VsInstallRoot=%%i" & goto :VsInstallRootDone
:VsInstallRootDone
...
:Usage
echo Usage: %BatchFile% [/rebuild^|/configure^|/update^] [/debug^|/release] [/no-node-reuse] [/no-multi-proc]
echo.
echo   Build targets:
