...
namespace Merq
{
	public class CommandBusSpec
	{
		...
		{
			new CommandBus(Mock.Of<IServiceProvider>(x => 
		...
		{
			var bus = new CommandBus(Mock.Of<IComponentModel>());
		...
		{
			Assert.Throws<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).CanHandle (null));
		}

		[Fact]
		public void when_can_execute_with_null_command_then_throws ()
		{
			Assert.Throws<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).CanExecute<Command> (null));
		}

		[Fact]
		public void when_execute_with_null_command_then_throws ()
		{
			Assert.Throws<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).Execute (default (Command)));
		}

		[Fact]
		public void when_execute_result_with_null_command_then_throws ()
		{
			Assert.Throws<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).Execute (default (CommandWithResult)));
		}

		[Fact]
		public async Task when_executeasync_with_null_command_then_throws ()
		{
			await Assert.ThrowsAsync<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).ExecuteAsync (default (AsyncCommand), CancellationToken.None));
		}

		[Fact]
		public async Task when_executeasync_result_with_null_command_then_throws ()
		{
			await Assert.ThrowsAsync<ArgumentNullException> (() => new CommandBus(Mock.Of<IComponentModel>()).ExecuteAsync (default (AsyncCommandWithResult), CancellationToken.None));
		}
